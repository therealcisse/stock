...
class VoidForm extends React.Component {
  onVoid = async () => {
    ...
    const { id, voidQuotation } = this.props;

    const { data: { voidQuotation: { error } } } = await voidQuotation(id);
        ...
        {this.state.loading ? (
          ...
          [
            <DialogContent key="content">
              <DialogContentText>
                Cette opération ne s'affichira plus.
              </DialogContentText>
            </DialogContent>,
...
export default compose(withStyles(styles), injectIntl, DataLoader.voidQuotation)(
  VoidForm,
);

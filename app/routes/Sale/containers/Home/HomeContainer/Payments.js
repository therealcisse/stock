...
import DelSalePayment from './DelSalePayment';
...
class Payments extends React.PureComponent {
  state = {
    order: Store.get(`sale.payments.${this.props.n.sale.id}.order`, 'desc'),
    orderBy: Store.get(
      `sale.payments.${this.props.n.sale.id}.orderBy`,
      'dateCreated',
    ),
  ...
  handleRequestSort = (event, property) => {
    this.setState(prevState => {
      ...
      Store.set({
        [`sale.payments.${this.props.n.sale.id}.order`]: order,
        [`sale.payments.${this.props.n.sale.id}.orderBy`]: orderBy,
      });
  ...
  render() {
    ...
    if (n.sale.payments.length === 0) {
      return null;
    }

    const items = this.getItems(n.sale.payments);
